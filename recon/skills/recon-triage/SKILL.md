...
6. **Gate (single approval).** AskUserQuestion shows the comment draft AND the attachment manifest: the two filenames (`recon-dossier-<TICKET>.html`, `recon-artifacts-<TICKET>.zip`), their sizes, and the bundle file count from the `MANIFEST:` line. Options: `Post to Jira now (comment + 2 attachments)` / `Edit first` / `Don't post`. One "post" answer authorizes the comment AND both attachments. NEVER post or attach without it (rule 2). On `Edit first`: after ANY change to `comment.txt`, re-run `verify-comment-shape.sh` until `shape: clean`, then re-show this gate — the posted bytes are always shape-verified and freshly approved. Any edit that changes `triage.yaml` blockers also requires re-running posting-path steps 3–5 (dossier render, package) so the attached dossier and zip match the comment being approved.
