...
| `recon-report/` | D | Verifies generated evidence, then fills its dossier template from current-run artifacts. Always renders locally; saves the complete current run only from an explicit store config; publishes only when `publish_once` is available. |
