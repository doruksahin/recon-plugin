...
| `triage-tools.py` | Shared engine behind verify-triage.sh, render-comment.sh, and verify-post-gate.sh: hand-rolled parser for the fixed `triage.yaml` and `post-gate.yaml` schemas (no PyYAML), disposition derivation, quote corpus + normalization, comment rendering, posting-gate exchange validation. Not invoked directly. |
