...
| 2 | Code discovery | recon-discovery | `triage/triage.yaml` with `disposition: READY` | ID-bearing contract → routing → required repro + verification → brief → pre-gate verification → rail-rendered gate questions → gate (exchange recorded verbatim) → post-gate verification → **STOP** (handoff quoted verbatim from `route/routing.yaml`, never executed) |
...
| `triage/jira/post-gate.yaml` | triage | `verify-post-gate.sh`, `derive-state.sh`, audit | ordered `exchanges` — one per presentation, each with the user's `answer_verbatim` and an `outcome` of `posted`/`edited`/`declined`; the terminal entry is last |
...
| Posting-gate render | posting path, after `package-artifacts.sh`, and again after every Edit-loop change | `render-post-gate.sh <TICKET> <zip path>` emits `triage/jira/post-gate-questions.txt`; the gate presents those bytes verbatim, never a paraphrase |
