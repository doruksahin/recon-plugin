...
cp -R "$TMP/blocked" "$TMP/file-symlink-leaf"
mkdir -p "$TMP/file-symlink-leaf/outside"
printf 'repository mapping closes this technical choice\n' >"$TMP/file-symlink-leaf/outside/mapping.txt"
rm "$TMP/file-symlink-leaf/source/mapping.txt"
ln -s "$TMP/file-symlink-leaf/outside/mapping.txt" "$TMP/file-symlink-leaf/source/mapping.txt"
expect_fail 'file evidence path contains a symlink: mapping.txt' env RECON_ROOT="$TMP/file-symlink-leaf" RECON_SOURCE_ROOT="$TMP/file-symlink-leaf/source" bash "$VERIFY" TEST-1

cp -R "$TMP/blocked" "$TMP/file-symlink-ancestor"
mkdir -p "$TMP/file-symlink-ancestor/outside"
printf 'repository mapping closes this technical choice\n' >"$TMP/file-symlink-ancestor/outside/mapping.txt"
ln -s "$TMP/file-symlink-ancestor/outside" "$TMP/file-symlink-ancestor/source/linked"
