...
| `test-dossier-store.sh` | Hermetic current-workspace storage-adapter controls: exact task-packet-store pin, support-file inclusion, archive pruning, v1 preservation on repeat, success-receipt locations, and empty-stdout failures at validation, checkpoint, and locate boundaries. |
