...
| `render-decree-reports.py` | Complete generated-view owner for tracked Decree completion reports: regenerates through Decree, canonicalizes portable document and stable transition identities, and fails `--check` on report-set or whole-body drift after ignoring only the Generated timestamp. |
| `test-decree-reports.sh` | Isolated complete-report controls proving body and Document mutations fail, timestamp-only variation passes, and clean regeneration is byte-equivalent modulo that timestamp. |
