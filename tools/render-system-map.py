#!/usr/bin/env python3
"""Render the source-linked overview of Recon's three operating layers."""
...
def ref(ref_id: str, title: str, relative: str, needle: str) -> dict:
    ...
    for line, text in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        if needle in text:
            return {"id": ref_id, "title": title, "path": relative, "line": line, "hash": digest(path)}
...
<style>
:root{{color-scheme:light dark;--bg:light-dark(#f7f7f4,#10131a);--panel:light-dark(#fff,#181d27);--text:light-dark(#17202a,#edf2fa);--muted:light-dark(#536274,#b8c4d3);--line:light-dark(#d9e0e8,#344052);--runtime:light-dark(#176b53,#72d6af);--lab:light-dark(#17629b,#79c4ff);--loop:light-dark(#7a4c10,#f2bd6b)}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--text);font:16px/1.5 system-ui,sans-serif}}main{{max-width:1120px;margin:auto;padding:42px 24px 80px}}h1{{font-size:clamp(2rem,5vw,4rem);line-height:1;margin:0 0 12px}}h2{{margin:0 0 10px}}p{{margin:0 0 12px}}.lede{{max-width:75ch;color:var(--muted);font-size:1.08rem}}.now,.card,.step{{background:var(--panel);border:1px solid var(--line);border-radius:14px}}.now{{margin:28px 0;padding:18px;border-left:5px solid var(--loop)}}.lanes,.grid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:15px}}.card{{padding:18px}}.runtime{{border-top:5px solid var(--runtime)}}.lab{{border-top:5px solid var(--lab)}}.loop{{border-top:5px solid var(--loop)}}.flow{{display:grid;gap:10px;margin-top:18px}}.step{{padding:14px}}.step b{{display:block;margin-bottom:5px}}.io{{display:grid;grid-template-columns:90px 1fr;gap:4px 12px;font-size:.93rem}}.io strong{{color:var(--muted)}}section{{margin-top:48px}}table{{width:100%;border-collapse:collapse;font-size:.9rem}}th,td{{padding:10px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}}th{{color:var(--muted)}}code{{font-size:.88em}}a{{color:inherit;text-underline-offset:3px}}.arrow{{text-align:center;color:var(--muted);font-size:1.3rem}}.state{{font-size:1.15rem;font-weight:700}}@media(max-width:760px){{main{{padding:28px 16px 56px}}.lanes,.grid{{grid-template-columns:1fr}}}}@media print{{body{{background:#fff;color:#111}}.now,.card,.step{{box-shadow:none}}}}
</style></head><body><main>
<p>Generated system map · source-linked and drift-checked</p><h1>Recon has three separate flows.</h1>
<p class="lede">The shipped plugin handles a real Jira ticket. The laboratory tests one frozen skill snapshot. The improvement loop decides how we change that skill. They connect, but they must not be confused.</p>
...
<section><h2>System map</h2><div class="lanes"><article class="card runtime"><h3>1 · Shipped plugin</h3><p>Real ticket → evidence-backed decision → human-gated output.</p><p><a href="#runtime">See inputs and outputs</a></p></article><article class="card lab"><h3>2 · Replay laboratory</h3><p>Frozen case → fresh LLM submission → immutable score.</p><p><a href="#laboratory">See states</a></p></article><article class="card loop"><h3>3 · Improvement loop</h3><p>Baseline evidence → candidate → comparison → review.</p><p><a href="#improvement">See current attempt</a></p></article></div><p class="arrow">candidate skill revision flows from the improvement loop → laboratory → evidence back to the improvement loop</p></section>
